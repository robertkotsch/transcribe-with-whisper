...
.EXAMPLE
  .\Transcribe-Folder-Enhanced-Clean.ps1 "C:\Media"
  .\Transcribe-Folder-Enhanced-Clean.ps1 "C:\Media" -SkipExisting
  .\Transcribe-Folder-Enhanced-Clean.ps1 "C:\Media" -OnlyQA -HighQualityQuestions -MarkdownOutput
#>
...
$videos = Get-ChildItem -Path $SourceFolder -File -Recurse | Where-Object { $_.Extension -match '\.mp4$|\.mov$' }
...
foreach ($f in $videos) {
  ...
  if ($ShouldInsights) {
    ...
    if (Test-Path $refinedTxt) {
      $refinedStr = [string]$refinedTxt
      $ext = if ($MarkdownOutput) { ".md" } else { ".txt" }
      
      # Build paths with correct extensions
      $auditFile = $refinedStr.Replace('_refined.txt', "_audit$ext")
      $questionsFile = $refinedStr.Replace('_refined.txt', "_questions$ext")
      $answersFile = $refinedStr.Replace('_refined.txt', "_answers$ext")
